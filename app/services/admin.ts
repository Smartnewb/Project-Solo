...
const stats = {
  ...
  getCustomPeriodSignupCount: async (
    ...
    try {
      ...
      const response = await axiosServer.post(
        "/admin/stats/users/custom-period",
        requestData,
      );
  ...
  getUniversityStats: async (
    ...
    try {
      ...
      if (
        ...
        console.log(
          "대학 데이터 키:",
          Object.keys(response.data.universities[0]),
        );
  ...
  getMonthlyWithdrawalCount: async () => {
    try {
      const response = await axiosServer.get(
        "/admin/stats/withdrawals/monthly",
      );
  ...
  // 사용자 지정 기간 탈퇴자 수 조회
  getCustomPeriodWithdrawalCount: async (
    startDate: string,
    endDate: string,
  ) => {
    try {
      ...
      const response = await axiosServer.post(
        "/admin/stats/withdrawals/custom-period",
        {
          startDate,
          endDate,
        },
  ...
  getDailyWithdrawalTrend: async () => {
    try {
      const response = await axiosServer.get(
        "/admin/stats/withdrawals/trend/daily",
      );
  ...
  getWeeklyWithdrawalTrend: async () => {
    try {
      const response = await axiosServer.get(
        "/admin/stats/withdrawals/trend/weekly",
      );
      return response.data || { data: [] };
    } catch (error) {
  ...
  getMonthlyWithdrawalTrend: async () => {
    try {
      const response = await axiosServer.get(
        "/admin/stats/withdrawals/trend/monthly",
      );
      return response.data || { data: [] };
    } catch (error) {
  ...
  // 사용자 지정 기간 탈퇴 추이 조회
  getCustomPeriodWithdrawalTrend: async (
    startDate: string,
    endDate: string,
  ) => {
    try {
      ...
      const response = await axiosServer.post(
        "/admin/stats/withdrawals/trend/custom-period",
        {
          startDate,
          endDate,
        },
  ...
  getWithdrawalReasonStats: async (startDate?: string, endDate?: string) => {
    try {
      ...
      const response = await axiosServer.get(
        "/admin/stats/withdrawals/reasons",
        { params },
      );
  ...
  getChurnRate: async () => {
    try {
      const response = await axiosServer.get(
        "/admin/stats/withdrawals/churn-rate",
      );
      return (
        response.data || {
          dailyChurnRate: 0,
...
const userAppearance = {
  ...
  getUsersWithAppearanceGrade: async (params: {
    ...
    try {
      ...
      if (params.universityName)
        queryParams.append("universityName", params.universityName);
      ...
      if (params.isLongTermInactive !== undefined)
        queryParams.append(
          "isLongTermInactive",
          params.isLongTermInactive.toString(),
        );
  ...
  sendProfileUpdateRequest: async (userId: string, message: string) => {
    try {
      ...
      const response = await axiosServer.post(
        "/admin/users/detail/profile-update-request",
        {
          userId,
          message,
        },
  ...
  setInstagramError: async (userId: string) => {
    try {
      ...
      const response = await axiosServer.post(
        "/admin/users/detail/instagram-error",
        {
          userId,
        },
      );
  ...
  resetInstagramError: async (userId: string) => {
    try {
      ...
      const response = await axiosServer.post(
        "/admin/users/detail/instagram-reset",
        {
          userId,
        },
      );
  ...
  getAppearanceGradeStats: async (region?: string, useCluster?: boolean) => {
    try {
      ...
      if (region) params.append("region", region);
      if (useCluster !== undefined)
        params.append("useCluster", useCluster.toString());
      ...
      if (typeof responseData === "object" && responseData !== null) {
        ...
        if (responseData.all && responseData.male && responseData.female) {
          ...
          formattedData.stats = grades.map((grade) => {
            ...
            const percentage =
              allStats.total > 0 ? (count / allStats.total) * 100 : 0;
              ...
              stats: grades.map((grade) => {
                ...
                const percentage =
                  responseData.male.total > 0
                    ? (count / responseData.male.total) * 100
                    : 0;
              ...
              stats: grades.map((grade) => {
                ...
                const percentage =
                  responseData.female.total > 0
                    ? (count / responseData.female.total) * 100
                    : 0;
          ...
          if (Array.isArray(responseData.stats)) {
            ...
            responseData.data &&
            Array.isArray(responseData.data.stats)
          ) {
          ...
          if (Array.isArray(responseData.genderStats)) {
            ...
            responseData.data &&
            Array.isArray(responseData.data.genderStats)
          ) {
          ...
          formattedData.genderStats = genderStatsData.map(
            (genderStat: { stats: any[]; gender: string }) => {
              // 각 성별별 총 사용자 수 계산
              const genderStatsArray = Array.isArray(genderStat.stats)
                ? genderStat.stats
                : [];
              const genderTotal = genderStatsArray.reduce(
                (sum: number, stat: { count: number }) =>
                  sum + (stat.count || 0),
                0,
              );
          ...
          },
          {
            ...
            stats: [
              ...
              },
              {
                ...
                percentage:
                  (testData.female.UNKNOWN / testData.female.total) * 100,
              },
            ],
          },
        ],
  ...
  getVerifiedUsers: async (params: {
    ...
    try {
      ...
      const response = await axiosServer.get(
        `/admin/users/verified?${queryParams.toString()}`,
      );
  ...
  approveUniversityVerification: async (userId: string) => {
    try {
      ...
      const response = await axiosServer.post(
        "/admin/university-verification/approve",
        {
          userId,
        },
      );
  ...
  rejectUniversityVerification: async (userId: string) => {
    try {
      ...
      const response = await axiosServer.post(
        "/admin/university-verification/reject",
        {
          userId,
        },
      );
  ...
  getBlacklistUsers: async (region?: string) => {
    try {
      ...
      const response = await axiosServer.get(
        `/admin/users/blacklist?${params.toString()}`,
      );
  ...
  releaseFromBlacklist: async (userId: string) => {
    try {
      ...
      const response = await axiosServer.patch(
        `/admin/users/${userId}/blacklist/release`,
      );
  ...
  revokeUserApproval: async (userId: string, revokeReason: string) => {
    try {
      ...
      const response = await axiosServer.patch(
        `/admin/users/approval/${userId}/revoke-approval`,
        {
          revokeReason,
        },
      );
...
const profileImages = {
  ...
  approveProfileImage: async (userId: string) => {
    try {
      ...
      const response = await axiosServer.post(
        `/admin/profile-images/users/${userId}/approve`,
      );
  ...
  approveIndividualImage: async (imageId: string) => {
    try {
      ...
      const response = await axiosServer.post(
        `/admin/profile-images/${imageId}/approve`,
      );
  ...
  rejectIndividualImage: async (imageId: string, rejectionReason: string) => {
    try {
      ...
      const response = await axiosServer.post(
        `/admin/profile-images/${imageId}/reject`,
        {
          rejectionReason,
        },
      );
...
const matching = {
  ...
  getMatcherHistory: async (
    ...
    try {
      ...
      const response = await axiosServer.get(
        "/admin/matching/matcher-history",
        { params },
      );
  ...
  getUnmatchedUsers: async (
    ...
    try {
      ...
      const response = await axiosServer.get(
        "/admin/matching/unmatched-users",
        {
          params,
        },
      );
  ...
  getMatchingStats: async (
    ...
    try {
      ...
      const stats =
        university && universityStats[university]
...
const pushNotifications = {
  filterUsers: async (
    ...
    try {
      const response = await axiosServer.post(
        "/admin/push-notifications/filter-users",
        {
          ...filters,
          page,
          limit,
...
const aiChat = {
  ...
  getSessions: async (params: {
    ...
    try {
      ...
      const response = await axiosServer.get(
        `/admin/ai-chat/sessions?${queryParams.toString()}`,
      );
  ...
  getMessages: async (sessionId: string) => {
    try {
      const response = await axiosServer.get(
        `/admin/ai-chat/messages?sessionId=${sessionId}`,
      );
...
const backgroundPresets = {
  ...
  uploadAndCreate: async (
    ...
    try {
      ...
      const response = await fetch(
        `${baseURL}/admin/background-presets/upload-and-create`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  create: async (data: CreatePresetRequest): Promise<BackgroundPreset> => {
    try {
      ...
      const response = await axiosNextGen.post(
        "/admin/background-presets",
        data,
      );
  ...
  update: async (
    ...
    try {
      ...
      const response = await axiosNextGen.put(
        `/admin/background-presets/${id}`,
        data,
      );
...
const cardNews = {
  uploadSectionImage: async (imageFile: File): Promise<UploadImageResponse> => {
    try {
      ...
      const response = await fetch(
        `${baseURL}/admin/background-presets/upload`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  create: async (data: CreateCardNewsRequest): Promise<AdminCardNewsItem> => {
    try {
      ...
      const response = await axiosNextGen.post<AdminCardNewsItem>(
        "/admin/posts/card-news",
        data,
      );
  ...
  get: async (id: string): Promise<AdminCardNewsItem> => {
    try {
      ...
      const response = await axiosNextGen.get<AdminCardNewsItem>(
  ...
  getList: async (
    ...
    try {
      ...
      const response = await axiosNextGen.get<AdminCardNewsListResponse>(
        "/admin/posts/card-news",
        {
          params: {
            page,
  ...
  update: async (
    id: string,
    data: UpdateCardNewsRequest,
  ): Promise<AdminCardNewsItem> => {
    try {
  ...
  publish: async (
    id: string,
    data?: PublishCardNewsRequest,
  ): Promise<PublishCardNewsResponse> => {
    try {
    ...
    try {
      ...
      const token =
        typeof window !== "undefined"
          ...
          : null;
      const baseURL =
        process.env.NEXT_PUBLIC_API_URL || "http://localhost:8045/api";
...
const deletedFemales = {
  getList: async (page: number = 1, limit: number = 20) => {
    try {
      const response = await axiosServer.get<DeletedFemalesListResponse>(
        "/admin/deleted-females",
        {
          params: { page, limit },
        },
      );
...
const banners = {
  ...
  update: async (id: string, data: UpdateBannerRequest): Promise<Banner> => {
    try {
      const response = await axiosServer.patch<Banner>(
        `/admin/banners/${id}`,
        data,
      );
      return response.data;
...
const dormantLikes = {
  ...
  getActionLogs: async (
    ...
    try {
      const response = await axiosServer.get<ActionLogsResponse>(
        "/admin/dormant-likes/logs",
        {
...
const appleRefund = {
  getList: async (params: AppleRefundListParams = {}) => {
    try {
      const response = await axiosServer.get<AppleRefundListResponse>(
        "/admin/apple-refund",
        {
          params: {
            page: params.page || 1,
...
const momentQuestions = {
  ...
  getList: async (
    ...
    try {
      ...
      if (params.translationStatus)
        queryParams.translationStatus = params.translationStatus;

      const response = await axiosServer.get<QuestionListResponse>(
        "/admin/questions",
        { params: queryParams },
      );
  ...
  getDetail: async (id: string): Promise<QuestionDetail> => {
    try {
      ...
      const response = await axiosServer.get<QuestionDetail>(
        `/admin/questions/${id}`,
      );
  ...
  update: async (
    ...
    try {
      ...
      const response = await axiosServer.put<QuestionDetail>(
        `/admin/questions/${id}`,
        data,
      );
  ...
  translate: async (
    data: TranslateQuestionsRequest,
  ): Promise<TranslateQuestionsResponse> => {
    try {
