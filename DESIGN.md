...
**`button-primary`** - Rausch fill, white text, 8px radius, 14x24px padding, 48px height, weight 500. The most common CTA across the system: "Reserve", "Continue", "Search", account-flow primaries.
...
**`button-pill-rausch`** - A pill-shaped Rausch CTA used on featured cells (e.g., "Become a host" sub-CTA) - 9999px radius, 10x20px padding, 14px label.
...
**`search-orb`** - The circular Rausch orb terminating the right edge of the search bar. 48x48px, fully rounded, white magnifying-glass icon centered. The hottest single color moment on the homepage.
...
**`date-picker-day`** - A 40x40px circular cell carrying the day number in `{typography.body-sm}`. Default state is transparent fill, ink text.
...
**`text-input`** - White surface, 1px hairline outline, `{rounded.sm}` 8px radius, 56px height, 14x12px padding. Stacked label above (in `{typography.caption}` muted), placeholder text in `{typography.body-md}` muted. On focus, the border thickens to 2px ink and the border color flips to `{colors.ink}` - no glow, no ring.
...
- Heart save button is 32x32px circular - borderline for AAA but compensated by a generous 12px padding inside the photo card.
