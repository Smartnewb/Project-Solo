...
function StatCard({ label, value, color, bgColor, subValue, subLabel }: StatCardProps) {
	return (
		<Box
			sx={{
				p: 2,
...
export default function SignupStatsDashboard({
	...
	const formatMonthlyData = (data: MonthlySignupTrendItem[]): ChartDataItem[] => {
	...
	const fetchCustomPeriodData = async () => {
		...
		try {
			...
			if (error.response?.status === 401) {
				...
				setCustomError(
					error.response?.data?.message || error.message || '데이터를 불러오는데 실패했습니다.',
				);
			}
		} finally {
	...
	const currentColor = TAB_CONFIG.find((t) => t.id === activeTab)?.color || '#8b5cf6';
			...
			<Box className="flex items-center justify-between mb-6 flex-wrap gap-3">
              ...
              `}
							style={{
								...
								boxShadow: activeTab === tab.id ? `0 4px 14px ${tab.color}40` : undefined,
							}}
						>
							{tab.label}
						</button>
				...
				<>
					...
					<Box className="flex gap-3 mb-6 flex-wrap">
						<StatCard
							...
							subValue={activeTab === 'custom' ? usersWithProfileImage : undefined}
							subLabel="사진"
						/>
						<StatCard label="평균" value={stats.avg} color="#3b82f6" bgColor="#eff6ff" />
						<StatCard label="최대" value={stats.max} color="#10b981" bgColor="#ecfdf5" />
						<StatCard label="최소" value={stats.min} color="#f59e0b" bgColor="#fffbeb" />
					</Box>
